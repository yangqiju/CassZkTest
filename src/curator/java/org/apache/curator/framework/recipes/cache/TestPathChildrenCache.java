...
public class TestPathChildrenCache extends BaseClassForTests {
	...
	public void testAsyncInitialPopulation() throws Exception {
		...
		try {
			...
			PathChildrenCacheEvent event = events.poll(timing.forWaiting()
					.seconds(), TimeUnit.SECONDS);
			Assert.assertEquals(event.getType(),
					PathChildrenCacheEvent.Type.CHILD_ADDED);

			event = events
					.poll(timing.forWaiting().seconds(), TimeUnit.SECONDS);
			Assert.assertEquals(event.getType(),
	...
	public void testChildrenInitialized() throws Exception {
		...
		try {
			...
			Assert.assertEquals(cache.getCurrentData().get(0).getData(),
					"1".getBytes());
			Assert.assertEquals(cache.getCurrentData().get(1).getData(),
					"2".getBytes());
			Assert.assertEquals(cache.getCurrentData().get(2).getData(),
					"3".getBytes());
		} finally {
			CloseableUtils.closeQuietly(cache);
			CloseableUtils.closeQuietly(client);
			...
			cache.getListenable().addListener(new PathChildrenCacheListener() {
				...
				public void childEvent(CuratorFramework client,
					...
					if (event.getType() == PathChildrenCacheEvent.Type.CHILD_REMOVED) {
						...
						Assert.assertTrue(postRemovedLatch.await(10,
								TimeUnit.SECONDS));
					} else {
						try {
							Assert.assertEquals(event.getData().getData(),
									"two".getBytes());
						} finally {
							dataLatch.countDown();
						}
		...
		try {
			...
			Future<Object> future = service.submit(new Callable<Object>() {
				...
				public Object call() throws Exception {
					...
					// simulate another process adding a node while we're
					// rebuilding
					...
					// simulate another process removing a node while we're
					// rebuilding
					...
					Assert.assertEquals(childData.getData(),
							"original".getBytes());
			...
			Assert.assertEquals(cache.getCurrentData("/test/snafu").getData(),
					"grilled".getBytes());
	...
	public void testIssue27() throws Exception {
		...
		try {
			...
			final List<PathChildrenCacheEvent.Type> events = Lists
					.newArrayList();
			final Semaphore semaphore = new Semaphore(0);
			PathChildrenCache cache = new PathChildrenCache(client, "/base",
					true);
			cache.getListenable().addListener(new PathChildrenCacheListener() {
				@Override
				public void childEvent(CuratorFramework client,
	...
	public void testBasics() throws Exception {
		...
		try {
			...
			Assert.assertEquals(new String(cache.getCurrentData("/test/one")
					.getData()), "sup!");
	...
	public void testBasicsOnTwoCachesWithSameExecutor() throws Exception {
		...
		try {
			...
			Assert.assertEquals(new String(cache.getCurrentData("/test/one")
					.getData()), "sup!");
			Assert.assertEquals(new String(cache2.getCurrentData("/test/one")
					.getData()), "sup!");
