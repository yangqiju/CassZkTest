	...
	public void testMultiClients() throws Exception {
		...
		try {
				...
				Future<List<Integer>> future = service
						.submit(new Callable<List<Integer>>() {
							...
							public List<Integer> call() throws Exception {
								final List<Integer> countList = Lists
										.newArrayList();
								...
								SharedCount count = new SharedCount(client,
										"/count", 10);
								...
								count.addListener(new SharedCountListener() {
									@Override
									public void countHasChanged(
											SharedCountReader sharedCount,
			...
			for (int i = 0; i < 100; ++i) {
				...
				Assert.assertTrue(semaphore.tryAcquire(CLIENT_QTY, 10,
						TimeUnit.SECONDS));
			}
			count.setCount(-1);
