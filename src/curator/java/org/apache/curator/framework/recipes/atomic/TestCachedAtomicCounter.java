...
public class TestCachedAtomicCounter extends BaseClassForTests {
	...
	public void testWithError() throws Exception {
		...
		client.start();
		try {
			AtomicValue<Long> value = new MutableAtomicValue<Long>(0L,
					(long) FACTOR, true);
			final AtomicReference<AtomicValue<Long>> fakeValueRef = new AtomicReference<AtomicValue<Long>>(
					value);
			DistributedAtomicLong dal = new DistributedAtomicLong(client, "/",
				...
				@Override
				public AtomicValue<Long> compareAndSet(Long expectedValue,
						Long newValue) throws Exception {
					return fakeValueRef.get();
				}
			};
			...
			for (int i = 0; i < FACTOR; ++i) {
				...
				if (i == 0) {
					MutableAtomicValue<Long> badValue = new MutableAtomicValue<Long>(
							0L, 0L);
