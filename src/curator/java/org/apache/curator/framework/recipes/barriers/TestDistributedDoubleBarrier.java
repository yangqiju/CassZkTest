...
public class TestDistributedDoubleBarrier extends BaseClassForTests {
			...
			Future<Void> future = service.submit(new Callable<Void>() {
				...
				public Void call() throws Exception {
					...
					try {
						...
						Assert.assertTrue(barrier.enter(timing.seconds(),
								TimeUnit.SECONDS));
						...
						Assert.assertTrue(barrier.leave(timing.seconds(),
								TimeUnit.SECONDS));
	...
	public void testOverSubscribed() throws Exception {
		...
		ExecutorService service = Executors.newCachedThreadPool();
		ExecutorCompletionService<Void> completionService = new ExecutorCompletionService<Void>(
				service);
		try {
			client.start();
			...
			Assert.assertTrue(semaphore.tryAcquire(QTY + 1, timing.seconds(),
					TimeUnit.SECONDS)); // wait until all QTY+1 barriers are
										// trying to enter
			latch.countDown();
