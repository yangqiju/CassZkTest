...
public class TestFramework extends BaseClassForTests {
	...
	public void testConnectionState() throws Exception {
		...
		try {
			...
			client.start();
			Assert.assertEquals(
					queue.poll(timing.multiple(4).seconds(), TimeUnit.SECONDS),
	...
	public void testNamespaceWithWatcher() throws Exception {
		...
		CuratorFramework client = builder
				.connectString(server.getConnectString()).namespace("aisa")
	...
	public void testEnsurePathWithNamespace() throws Exception {
		...
		CuratorFramework client = builder
				.connectString(server.getConnectString())
				.retryPolicy(new RetryOneTime(1)).namespace(namespace).build();
		client.start();
		try {
			EnsurePath ensurePath = new EnsurePath("/pity/the/fool");
			ensurePath.ensure(client.getZookeeperClient());
			Assert.assertNull(client.getZookeeperClient().getZooKeeper()
					.exists("/jz/pity/the/fool", false));

			ensurePath = client.newNamespaceAwareEnsurePath("/pity/the/fool");
			ensurePath.ensure(client.getZookeeperClient());
			Assert.assertNotNull(client.getZookeeperClient().getZooKeeper()
					.exists("/jz/pity/the/fool", false));
		} finally {
			client.close();
		}
	...
	public void testNamespace() throws Exception {
		...
		try {
			...
			bytes = client.nonNamespaceView().getData()
					.forPath("/" + namespace + "/test/child");
			Assert.assertEquals(bytes, "hey".getBytes());
		} finally {
			client.close();
		}
	...
	public void testBackgroundDeleteWithChildren() throws Exception {
		...
		try {
			...
			CountDownLatch latch = new CountDownLatch(1);
			client.delete().deletingChildrenIfNeeded().inBackground(latch)
					.forPath("/one/two");
			Assert.assertTrue(latch.await(10, TimeUnit.SECONDS));
	...
	public void testDeleteWithChildren() throws Exception {
		...
		try {
			...
			Assert.assertNull(client.checkExists().forPath(
	...
	public void testBackgroundGetDataWithWatch() throws Exception {
		...
		try {
			...
			client.getData().watched().inBackground(backgroundLatch)
					.forPath("/test");
	...
	public void testBackgroundCreate() throws Exception {
		...
		try {
			...
			CountDownLatch latch = new CountDownLatch(1);
			client.create().inBackground(latch)
					.forPath("/test", new byte[] { 1, 2, 3 });
			Assert.assertTrue(latch.await(10, TimeUnit.SECONDS));
		} finally {
			client.close();
		}
	...
	public void testCreateModes() throws Exception {
		...
		try {
			...
			client.create().forPath("/test", writtenBytes); // should be
															// persistent
			...
			realPath = client.create()
					.withMode(CreateMode.EPHEMERAL_SEQUENTIAL)
					.forPath("/eseq", writtenBytes);
