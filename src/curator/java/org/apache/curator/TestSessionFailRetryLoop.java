...
public class TestSessionFailRetryLoop extends BaseClassForTests {
	...
	public void testRetry() throws Exception {
		...
		SessionFailRetryLoop retryLoop = client
				.newSessionFailRetryLoop(SessionFailRetryLoop.Mode.RETRY);
		retryLoop.start();
		try {
			client.start();
					...
					RetryLoop.callWithRetry(client, new Callable<Void>() {
						...
						public Void call() throws Exception {
							if (firstTime.compareAndSet(true, false)) {
								Assert.assertNull(client.getZooKeeper().exists(
										"/foo/bar", false));
	...
	public void testRetryStatic() throws Exception {
		...
		try {
			...
			SessionFailRetryLoop.callWithRetry(client,
					SessionFailRetryLoop.Mode.RETRY, new Callable<Object>() {
						@Override
						public Object call() throws Exception {
							RetryLoop.callWithRetry(client,
									new Callable<Void>() {
										@Override
										public Void call() throws Exception {
											if (firstTime.compareAndSet(true,
													false)) {
												Assert.assertNull(client
														.getZooKeeper().exists(
																"/foo/bar",
																false));
												KillSession.kill(client
														.getZooKeeper(), server
														.getConnectString());
												client.getZooKeeper();
												client.blockUntilConnectedOrTimedOut();
	...
	public void testBasic() throws Exception {
		...
		final CuratorZookeeperClient client = new CuratorZookeeperClient(
				...
				timing.connection(), null, new RetryOneTime(1));
		SessionFailRetryLoop retryLoop = client
				.newSessionFailRetryLoop(SessionFailRetryLoop.Mode.FAIL);
		retryLoop.start();
		try {
			client.start();
			try {
	...
	public void testBasicStatic() throws Exception {
		...
		try {
			...
			try {
				SessionFailRetryLoop.callWithRetry(client,
						SessionFailRetryLoop.Mode.FAIL, new Callable<Object>() {
							@Override
							public Object call() throws Exception {
								RetryLoop.callWithRetry(client,
										new Callable<Void>() {
